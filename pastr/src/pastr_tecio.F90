...
module pastr_tecio
  ...
  Interface tecbin
    ...
    module procedure writetecbin3d_patch
  ...
  subroutine writetecbin3d_patch(filename,patch)

    use pastr_multiblock_type, only : patch_type
    
    ! arguments
    character(len=*),intent(in) :: filename
    type(patch_type),intent(in),dimension(:) :: patch(:)
    ...
    do n=1,size(patch)
      ...
      imax  =size(patch(n)%x,1)
      jmax  =size(patch(n)%x,2)
      kmax  =size(patch(n)%x,3)
      nbrvar=patch(n)%nvar+3
      ...
      v(:,:,:,2) =patch(n)%x(:,:,:,2)
      v(:,:,:,3) =patch(n)%x(:,:,:,3)
      v(:,:,:,4:)=real(patch(n)%var(:,:,:,:))
      vname(1)='x'
      vname(2)='y'
      vname(3)='z'
      vname(4:)=patch(n)%varname(:)

      write(bname,'(i4.4)')n
      
      call tec_data_writer(filename=filename//'-patch'//bname//'.plt',varname=vname,var=v)
  ...
  subroutine writetecbin3d1var_char(filename,var1,var1name,x,y,z)
    ! 
    character(len=*),intent(in) :: filename
    character(len=1),intent(in) :: var1(:,:,:)
    character(len=*),intent(in) :: var1name
    real(8),intent(in),optional :: x(:,:,:),y(:,:,:),z(:,:,:)
    !
    ! local data
    ...
    do j=1,jmax
    do i=1,imax
      if(var1(i,j,k)=='f') then
        var(i,j,k,1)=0.0
      elseif(var1(i,j,k)=='b') then
        var(i,j,k,1)=1.0
      elseif(var1(i,j,k)=='s') then
        var(i,j,k,1)=-1.0
      endif
    enddo
    enddo
    ...
    !
    deallocate(var)
  end subroutine writetecbin3d1var_char
