...
更新日期：2026-09-20。当前代码基线：`feature/gpu_dev` 当前工作树。
...
NP=2 x-slab 激波管短步门槛已经通过。独立有限速率双温平板推进器的正性、流向
步长、17/33/65 点法向网格及累计推进门槛已通过；ASTR CPU/GPU 同站位物理对比、
正常激波松弛、激波格式物理边界、长时收敛和有限速率空气 SBLI 尚未完成。
...
反应流 `lfilter=t` 尚未通过：两步 HBL 的 CPU `z` 挤出缩放误差为
`1.484677e-5`，而修改前旧可执行文件为 `1.484665e-5`。该匹配失败说明问题独立
于本次无参数比例，位于当前反应流十阶滤波路径。在定位并通过 `2e-10` 门槛前，
不得将反应流滤波列为已验证能力，也不启动其长步、restart 或性能测试。
...
6. 固定 air5 已分别实现共享面扩散正性限制和组分专用对流 FCT。后者保持密度、
   ...
   常驻验收，关闭 A1-R3 与 C5-6A1 后再晋级 C5-6B1。
