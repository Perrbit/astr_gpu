...
## 2026-09-26 全流场补偿生命周期及 L1 数值门槛通过

用户批准先接入全场化学、SSP-RK、边界及 restart，再启用长时验证。
新增候选开关 `ASTR_AIR5_COMPENSATION=on`，默认 `off`。当前代码已接入
CPU/GPU 化学调用及 11 个守恒量的 SSP-RK 余量传播，原有 metric-weighted
`qsave` 继续用于限制器，不改变其单位。短步、重启及内存门槛已通过，
不等于长时数值验证或 SBLI 物理验收已通过。

首次接入版本在重复节点余量尚未合并时写全局 checkpoint，导致分段重启
相对连续运行出现约 1e-18 的动量差异。已在候选每个完整步末尾执行
高位/余量的重复节点合并，独立于文件输出频率，原始失败证据保留。

已验证：CPU/GPU 的 NP1、NP2 x-slab 短步逐场比较通过原有
`atol=1e-9, rtol=1e-10`，最大组成和误差 5.55e-16。
CPU 与 GPU 各自的连续/重启测试中，11 个保守量和 11 个余量均逐位一致。
双卡 memcheck 零错误，拒绝旧文件未授权初始化、关闭补偿读取补偿文件、
NaN 余量及自动 crash-fix。默认关闭路径与已有冻结程序在既定容差内一致，
不是跨版本逐位一致（最大原始量差异为压力 7.28e-12 Pa）。

生命周期证据：`tests/gpu_validation/out/air5_compensated_flow_canonical_20260926/`。
矩阵候选 `d10dd8a...` 完成后仅增加 crash-fix 拒绝保护；最终二进制
`fd724b07fede6992ecdcb2e7267569f9171ae3904a7f501ca3a03d1d23e17e2d`
复跑 CPU/GPU 重启回归，仍与已冻结连续结果逐位一致，GPU 同时通过 memcheck。

余量采用 `q - carry` 符号约定，化学失败时不提交局部 trial carry。
物理边界重建后清空对应节点的余量，MPI/周期重复节点则合成两侧余量及
原平均操作的舍入误差，不清空内部接口余量。候选通信使用私有 communicator，
GPU 仅为这一步传输面数据到主机；尚不作为通信性能优化结果。

检查点新增版本号、11 个原始保守量 `acq01..11` 和 11 个余量 `acc01..11`。
补偿续算在原始量重构之后恢复保守量，避免仅恢复余量却改变高位。
旧文件必须显式指定 `ASTR_AIR5_COMPENSATION_RESTART=initialize`，声明从零
余量启动新实验。候选暂限 AIR5 HBL/SBLI RK3，拒绝滤波、海绵层、浸入边界
及自动 crash-fix。当前完整流场 MPI 验证范围为本地 x-slab、host-staged。

L1 已启用补偿完成，输出在
`tests/gpu_validation/out/air5_compensated_L1_20260926/`。
同一旧 checkpoint 从 t=2.18 us 推进至 2.38 us，使用上述最终二进制。

| 时间步 | 完整更新数 | 末次更新 step | 最大记录 CFL | 结果 |
| --- | --- | --- | --- | --- |
| 2 ns | 100 | 1189 | 0.2864691 | 完成 |
| 1 ns | 200 | 1289 | 0.1432378 | 完成，跨过原 step1266 失败点 |
| 0.5 ns | 400 | 1489 | 0.0716198 | 完成，跨过历史候选 step1273/1359 失败点 |

各组首末步共 40 份 rank/stage 状态均无负组分或质量闭合门槛超限，
顺序求和的最大组成和误差为 5.551115e-16，未放宽 128 epsilon 门槛。
中间步骤由运行时物性和输运检查保护，不宣称保存了全部阶段快照。
同终点 2/1 ns 与 1/0.5 ns 对照的最大流向速度差分别为
6.47670e-6、8.03517e-7 m/s，最大温度差为 5.16067e-5、6.44130e-6 K，
最大压力差为 1.94434e-3、2.37884e-4 Pa。痕量组分差异并非全部单调减小，
这些结果不构成所有变量的形式阶数证明。

下一步为 L2 的 2/1 ns 同窗对照至 4.18 us，再按门槛进入 L3 至 12.18 us。
L2/L3 尚未运行。当前仍是无入射激波的 Mach4 平板数值验证，不是完整 SBLI
物理验收。下方“长时数值验证准备”保留为接入前历史记录。

## 2026-09-26 长时数值验证准备（接入前历史记录）

已复核 step1090、t=2.18 us 的 64x512x8 Mach4 无入射激波平板 checkpoint
及 SHA256。长时矩阵固定本地 NP2、2x1x1、FP64、滤波关闭，先做 2/1/0.5 ns
的 100/200/400 次更新同窗对照至 2.38 us，再逐级延长至 4.18 和 12.18 us。
后两层各保留 2/1 ns 对照，任一门槛失败即停止后续窗口。

**准备阶段禁止把它作为补偿修复验收启动**：当时 CPU/GPU 全场化学调用均未传入
补偿余量，SSP-RK 余量传播、边界/重复节点及 checkpoint 生命周期尚未接入。
旧 checkpoint 可作为明确声明余量为零的新实验初态，但不能伪装为保留了
历史低位的连续重启。不得用未实现的环境变量启动所谓补偿路径。

详细前置条件、时间/step 表、稀疏 I/O、同相位比较及预算见
[Mach4 计划](ASTR_AIR5_MACH4_SBLI_PLAN.md) 本日“长时数值验证准备”。
声明式配置：`tests/gpu_validation/air5_long_window_plan.json`，准备时标记为
`blocked-pending-full-flow-compensation`、`auto_launch=false`。
未运行新流场、未操作远程作业、未暂存或提交。

## 2026-09-26 自适应化学补偿单元验收 goal 完成

本轮 goal 限于可选 FP64 ROS-2 化学补偿，不包含空间 RK 或完整流场。
未修改上一轮 CPU/GPU 积分器，只扩展验证探针与独立参考检查。

新增 20 kPa、初始质量分数 `(0.55,0.15,0.10,0.12,0.08)`、
速度 `(2000,0,0) m/s` 的两组 20 ns 耦合化学轨迹。
初始 `T/Tv=6000/1000 K` 的普通/补偿 CPU/GPU 均接受 1981 步、拒绝 5 步，
`4000/4000 K` 均接受 1563 步、拒绝 5 步。

`check_ros2_compensation_radau.py` 从探针读取真实初态，复用已有
`Air5RadauReference`，没有新增 Python 流场推进器。八项端点对照均通过：

| 判据 | 最大误差/容差比或误差 | 门槛 |
| --- | --- | --- |
| 质量分数对 Radau | 9.014e-5（容差比） | <=1，rtol=1e-6、atol=1e-10 |
| T/Tv 对 Radau | 7.844e-4（容差比） | <=1，rtol=1e-7 |
| 质量/N/O 漂移，含普通路径 | 3.148e-15 | <=1e-14 |
| q5 EOS 往返相对差 | 1.297e-16 | <=32 epsilon |

补偿两组 GPU 末态质量及元素漂移在 FP64 求和分辨率内为零，不代表精确
算术守恒。q5 仍为化学调用的只读参数，EOS 往返不是独立空间能量验收。
NaN/Inf 时间参数、容差、余量及 NaN 组分输入均正确拒绝，并按位保留
输入状态和余量；原拒步恢复、失败回滚及相同子步跨调用一致性继续通过。

复核并复用 9 月 25 日的 20000 次调用六路径日志：补偿最大组成和误差
2.22045e-16，普通低温对照超出 128 epsilon。原探针二进制另存为
`out/air5_ros2_compensation_acceptance_20260926/previous_probe`。
本日 `sha256.txt` 是复用时补录的指纹，不冒称原运行时生成的清单。
源项/积分器未变，不重复该昂贵测试。

根 CMake 主程序及扩展探针编译通过，59 项回归和 11 个子测试通过。
新证据位于 `tests/gpu_validation/out/air5_ros2_compensation_acceptance_20260926/`：
`probe.log`、`radau.json`、`sha256.txt`，复现命令见测试 README。
扩展探针的 `memcheck.log` 确认零错误、零泄漏，涵盖新增刚性及非有限
输入路径。`default_gpu.log` 确认默认路径状态/诊断零不匹配、q5 零变化。
本轮 goal 的化学单元准入门槛全部通过，可选补偿仍不在生产流场中启用。

下一步仍为与 metric-weighted qsave 相容的 SSP-RK 舍入补偿验证，之后才是
全场存储与边界/MPI/filter/restart 生命周期。单元准入不关闭 Mach4
长时失败，不推荐生产启用，不声称性能收益。未暂存、提交或推送。

## 2026-09-25 GPU 自适应化学补偿：单元与事务门槛通过

在 CPU/GPU 的 ROS-2 固定步与自适应接口中增加可选 `compensation(6)`。
五组分部分密度及 Ev 使用逐阶段增量 Kahan 累加，物性/源项仍查询 FP64
主状态，不提高组成阈值，不截零、不归一化，不修改 q5。
固定步通过候选状态检查后才提交余量；自适应调用在局部副本上试算，
仅接受子步推进内部余量，整个请求区间成功后才返回调用方余量。
拒步、非法输入及尝试次数耗尽均保留调用前的状态与余量。

新增 CMake 目标 `chemistry_ros2_compensation_gpu_probe`，同前轮
1500/2000/3000 K、20 kPa 初态，每组执行 20000 次自适应调用，
每次区间 0.5 ns，CPU/GPU 分别比较普通和补偿路径。

| 初始 T/Tv | GPU 普通最大 abs(sum(Y)-1) | GPU 补偿最大 abs(sum(Y)-1) |
| --- | --- | --- |
| 1500 K | 4.21885e-13 | 1.11022e-16 |
| 2000 K | 2.02172e-13 | 2.22045e-16 |
| 3000 K | 3.77476e-15 | 2.22045e-16 |

GPU 补偿质量相对漂移不超过 4.001e-16，末态氮/氧元素漂移均通过
1e-14 门槛。CPU/GPU 状态对照、组分非负及 T/Tv 反演通过。
53 位尾数诊断中的零值不能解释为精确算术守恒。

- 同一后端相同子步序列，分两次调用与一次调用的状态和余量逐位一致。
- 6000 K / Tv=1000 K 混合组分测试中，CPU/GPU 均拒步 2 次、接受 7 次后成功。
- 强制首步拒绝后耗尽次数，以及先接受一个子步后耗尽次数，两者都完整回滚。
- 非法时间步完整回滚；非零输入余量用于上述事务检查。
- CPU 测试 18 项、11 个子测试通过，包括补偿轨迹与既有独立 Radau 参考比较。
- 41 项相关诊断/契约测试、默认 GPU ROS-2 探针及 `astr` 主程序编译通过。
- Compute Sanitizer memcheck 的 100 次调用短矩阵零错误、零泄漏，覆盖全部事务路径。

证据位于 `tests/gpu_validation/out/air5_ros2_compensation_adaptive_20260925/`
的 `long.log`、`smoke.log`、`memcheck.log`、`default_gpu.log`。
补偿只由探针启用，主程序调用仍省略该参数；未新增全场补偿数组。
接口与设备编译产物已有变化，本轮不声称默认路径性能不变。

**下一门槛**：先验证实际 SSP-RK 更新中当前状态和原始状态的余量传播，
明确 metric-weighted qsave 的单位与加权关系，再设计全场持久存储、
边界覆盖/滤波失效规则、MPI 重复节点一致性及 restart 保存协议。
本次没有复跑完整 Mach4 长窗口，C5-6B 长时问题仍未关闭，不推进物理验收。
未暂存、提交或推送，也未操作远程任务。

## 2026-09-25 单元 ROS-2 持久补偿诊断：有效，但未接入流场

新增顶层 CMake 目标 `chemistry_ros2_roundoff_probe`，调用实际 CPU ROS-2
阶段计算，在状态加法前取得两个加权阶段增量，对比原顺序、合并增量及
逐增量 Kahan 持久补偿。补偿仅在探针中保存六个 FP64 余量，不修改源项、
阈值、组分闭合、物性或生产推进方式。`src/chemistry_kinetics.F90` 仅增加
可选诊断输出 `update_terms`，失败时清零，原 candidate 表达式不变。

固定初态为五组分空气 `(0.767,0.233,0,0,0)`、20 kPa、`T=Tv`，
速度 `(2000,0,0) m/s`。每条路径进行 20000 次 `dt=0.5 ns` 化学更新，
固定 rho、动量和 q5，无输运、滤波或边界，不是原失败流场的精确单点重放。
每步检查正性、T/Tv 反演和生产 ROS-2 容差定义下的误差范数，不合格即停止。

| 初始 T/Tv | 原顺序最大 abs(sum(Y)-1) | 合并增量 | 持久补偿 | 原顺序首次超过 128 epsilon |
| --- | --- | --- | --- | --- |
| 1500 K | 4.21885e-13 | 3.28182e-13 | 0（FP64 求和分辨率内） | 第 1352 次 |
| 2000 K | 2.02172e-13 | 2.02172e-13 | 2.22045e-16 | 第 2225 次 |
| 3000 K | 3.77476e-15 | 1.77636e-15 | 2.22045e-16 | 未超过 |

补偿路径在三组实验中均未越过组成门槛，质量、氮元素、氧元素最大相对
漂移分别不超过 `3.001e-16 / 3.506e-16 / 4.396e-16`。此处 NVHPC 主机
求和为 53 位尾数，报告中的零不等于精确守恒。最大局部误差范数为
`2.128e-3 < 1`，q5 固定，所有最终组分非负且 T/Tv 反演成功。

证据：`tests/gpu_validation/out/air5_ros2_roundoff_probe_20260925/nvhpc_*.log`。
CPU ROS-2 回归为 17 项及 9 个子测试通过，包含 gfortran 独立构建的补偿
诊断、默认接口不变及失败输出清零。默认 GPU ROS-2 探针也通过，状态与
诊断码零不匹配，q5 零变化；这是原 GPU 路径回归，不是 GPU 补偿验证。
顶层 CMake 的 `astr` 主程序编译成功，另 41 项相关诊断与契约测试通过。

下一步依次验证 GPU 单元补偿、拒步/失败事务回滚及跨化学调用持久性，
再设计与 SSP-RK 原始状态相容的余量传播。物理面、MPI 重复节点、滤波、
restart 与显存预算尚未解决，不分配全场余量，不恢复长时物理验收。
补偿表达式依赖浮点运算次序，后续须在实际 GPU 编译优化配置下验证，
不能把本次主机探针结论直接外推到允许重结合的编译选项。
本轮没有 Git 提交，也没有修改 GPU 数值实现。

## 2026-09-25 共享算术修复尝试：未关闭长窗口门槛

用户已批准处理 CPU/GPU 共有的有限精度问题。本轮先保存 step1090 和
step1265 的完整阶段快照，再尝试两项不改变方程、阈值或保正预算的算术重排。
两项均未完成细步长门槛，已撤回生产源码中的候选改动，不宣称修复完成。

| 候选 | 1 ns x 200 | 0.5 ns x 400 |
| --- | --- | --- |
| 仅 RK 增量式更新 | 完成；36 份阶段状态通过；闭合残差最大 2.775519e-14 | rank0 / step1273 / RK2，status=3 |
| RK 增量式 + ROS-2 合并增量 | 未重复 | rank0 / step1359 / RK1，status=3 |

第一候选的 12 组 CPU NP1/NP2、GPU NP2 能量/组分激活、接触平衡及
低 N2 回归通过，360 对共享面逐位一致。第二候选的 CPU ROS-2 测试
15 项及 9 个子测试通过，GPU ROS-2 探针状态/诊断码匹配，质量漂移最大
6.064377e-16、q5 不变。这些局部门槛不能替代失败的多步主程序门槛。

证据在 `tests/gpu_validation/out/air5_composition_failure_20260925/stage_budget/`、
`air5_rk_increment_stress_20260925/`、`air5_rk_increment_window_20260925/` 和
`air5_increment_combined_window_20260925/`。最后目录保留 `candidate.patch`
及冻结二进制。第一/第二候选主程序 SHA256 分别为
`b059d4a51774a2b676e68da795b4430e80b976f5ecafdb23125cc4c2a042c89d` /
`d96ddc6f884b93bc68e66331267a8d0e756e8f3e54ba057be1bd3db58ccd58f3`。

新增 `check_air5_mass_closure_stages.py` 使用保存的 FP64 q 与扩展精度求和
诊断闭合残差，不积分流场、不修改状态。所检查节点的化学半步与 RK 更新
都贡献了舍入变化；只在两个完整步保存阶段，不能推断整个历史的算子占比。
41 项诊断及相关契约测试通过。

下一修复应单独设计持久舍入补偿，而不是继续变换括号或提高 128 epsilon。
先用 ASTR 的单元化学/更新探针验证补偿能够降低多步质量和元素漂移，再
评估全场接入。需要明确当前/原始 RK 状态的补偿余量、化学拒步回滚、
物理边界与 MPI 重复节点、滤波后的生命周期，以及 restart 是否保存余量。
仅补偿总密度和五组分的双份 FP64 余量就约需每节点 96 字节；不能未经
显存与重启语义评估直接全场分配。总能量和振动能相容性仍是独立门槛。
当前保持数值基线，不扩大长时、细化窗口或入射激波验证，不执行 Git 提交。

## 2026-09-25 组成失效定位：闭合误差跨越输运阈值

失败步定点重放及关闭物性复用对照已完成。两者均在 rank1 / step1266 /
RK1 的扩散通量阶段以 status=3 停止，14 份对应二进制快照逐位相同。
因此同状态复用不是本次故障原因，不回退已验证的优化。

失败前 `pre_rhs` 的全部物理节点组分非负。按实际对称缓存定义
`Y_s=q_s/rho` 检查，只有 rank1 内点 `(11,46,5)` 超出输运组成和阈值：
初始 step1090 该点 `sum(Y)-1=0`，step1266 化学半步前为
`-2.842170943040401e-14`，半步后及 pre_rhs 为
`-2.853273173286652e-14`。后者比 `128*epsilon` 的界多
`1.110223024625157e-16`。不是负组分或温压越界，PMIX 错误是 abort
后的伴随输出。化学半步是本次越过阈值的位置，不代表全部历史漂移
都由化学算子引起；化学和输运的累计份额仍需分解。

共享接口不一致：CPU/GPU 守恒状态恢复使用
`abs(sum(rho_s)-rho)<=1e-10*max(rho,1)`，而输运接口要求
`abs(sum(Y)-1)<=128*epsilon`。本轮直接复现的是 GPU；CPU 源码存在
相同约束差异，但未运行 CPU 长窗口，不能宣称 CPU 同步数也会失败。
当前不自动提高 128 epsilon、不截零、不归一化状态、不改成 N2 余量覆盖。
建议下一步先审定 CPU/GPU 共同的离散质量不变量保持方案，再实施数值修复，
同步覆盖保正、元素、总能量/振动能、痕量自身尺度与三步长长窗口门槛。

证据：`tests/gpu_validation/out/air5_composition_failure_20260925/`
下 `reuse_on` / `reuse_off` 的契约、日志和 step1266 阶段快照。
所有回放已结束，本轮未改求解器或验收阈值，未执行 Git 提交。

## 2026-09-25 长窗口恢复：组分有效性门槛再次阻塞

以 `276081a` 对应已验收二进制运行本地双卡 NP2 x-slab，保留 FP64、
显式同步、`symmetric_species + layered`、关闭滤波；原始量复用为
`chemistry`，归约为 `baseline`。从 step1090、t=2.18 us 的同一旧检查点
计划比较 2/1/0.5 ns 的 100/200/400 次更新，终点均为 2.38 us。
这是无入射激波平板的故障回放，不是新的物理预发展初场。

- 2 ns 的 100 次更新完成，36 份起止阶段状态通过原状态检查，最大
  质量闭合相对残差 1.883965e-14，记录到的最大 CFL 为 0.2864691。
- 1 ns 在 rank1、step1266、RK stage1 的扩散通量计算报 status=3，
  步起始时间约 2.356 us，未到匹配终点。此前记录最大 CFL 为 0.1432339。
  status=3 是 `chemistry_status_invalid_composition`，不是温压越界。
- 按首个数值失败停止原则，没有启动 0.5 ns、一个流经时间或入射激波。
  无法形成三档时间收敛结论，也不将粗步长成功解释为长时通过。
- 当前日志不能区分负组分、组分和偏差或混合扩散分母异常。输运查询的
  组成和容差是 128 epsilon，约 2.842171e-14，而快照状态检查采用
  2e-12；两项检查不可混称同一门槛。失败前 step1200 的 HDF 组成和
  最大偏差为 1.898481e-14，组分非负，这只是较早时刻证据。
  尚不能据此认定失败根因是舍入累积，也不能归因于原始量复用。

证据：`tests/gpu_validation/out/air5_symmetric_long_window_reported_20260925/`
的 `gpu_dt2`、`gpu_dt1`，各含 `contract.json`、`result.json` 和 `gpu/run.log`。
首次粗步长尝试在 `out/air5_symmetric_long_window_20260925/gpu_dt2/`，
因 checkpoint 间隔过大没有记录 CFL，已保留并以间隔 50 重跑；正式
1 ns 组间隔为 100。只在首末指定步保存阶段场，不进行每步大场归档。
下一步应在同源回放的 step1266 保存失败前实际组成与位置，区分物理面、
MPI 端点和内点；随后单独关闭原始量复用作对照。定位前不截零、不归一化、
不放宽容差、不修数值算法。本轮未改求解器、未提交 Git、无遗留计算进程。

## 2026-09-25 同状态复用与反应流归约本地验收完成

以 `7fd3823` 为基线。A 在化学半步之后立即复用
严格内点已有的原始量，边界/halo 重算，不保留跨 RK、滤波或 restart
缓存。B 将化学诊断的同算子归约由每半步四次压缩到两次，保留失败
屏障。两项默认关闭，CPU、FP64、显式同步、物理模型及默认
`full_state` 均不变，无新增全域设备工作数组。

候选二进制 SHA256：
`7e26451c85cfd1153c733d65d1805cd67d832e6504decafb51c78bf2ff705687`。
首版全域发射后跳过内点没有稳定收益，最终版将待重算外层紧凑映射，
每点只恢复一次。空复用区域退回原全量路径。

- 首版 A/B/组合的 Mach4 NP2 回放各 20 份场与冻结基线相同，三组各两份
  memcheck 零错误，保正/物性域检查通过，边界通量收支最大缩放残差
  1.389390e-16，各 18 对共享面逐位一致。
- 最终版周期反应 TGV 的 off/A/B/组合在 NP1、NP2 x/y/z 均通过
  CPU 对照、元素/质量/能量检查，280 份 GPU 场与首版差值为零。
  最终 A NP2、组合 NP1/NP2 的 Mach4 再验证共 50 份场与冻结基线
  差值为零，新增五份 rank memcheck 零错误。
- 组合在 full/scalar 滤波工作区、NP1/NP2 x/y/z 八组检查通过。
  零组分及 NO=1e-12 的过滤 restart 补测通过，每组 36 对共享面一致，
  最终版 restart 最大相对全局守恒漂移 8.900738e-15。
- 单 rank 注入化学失败状态后全局以 99 退出，无后续化学完成/输运
  快照。默认 full_state 回归及激波管 1536 个传感器标记界面回归通过。
- 首次零组分补测错误使用了要求严格正组分的旧 TGV 检查器，CPU
  初始精确零被判失败。保留原记录，不改旧检查器；补测使用非负性
  且保留其他原阈值，不容忍任何负组分，45 个相关单测通过。

本地双 RTX 4000 Ada，Mach4 无入射激波 64x512x8，dt=2ns，每配置
三轮交错运行，一步预热、三步完整化学+输运计时，取逐步最慢 rank。

| 配置 | NP1 中位数 s/步 | NP2 中位数 s/步 | NP1/NP2 耗时降低 | 双卡效率 |
| --- | ---: | ---: | ---: | ---: |
| 基线 | 1.127277 | 0.736057 | -- | 76.58% |
| A 同状态复用 | 1.107205 | 0.714084 | 1.78% / 2.99% | 77.53% |
| B 归约合并 | 1.126187 | 0.735619 | 未超波动 | 76.55% |
| A+B | 1.107789 | 0.716591 | 1.73% / 2.64% | 77.30% |

推荐在已验证 AIR5 路径显式启用 `ASTR_AIR5_PRIMITIVE_REUSE=chemistry`。
`ASTR_AIR5_CHEMISTRY_REDUCTIONS=packed` 将 NP2 每步 Allreduce 从
82 次减至 78 次，但无独立可辨整步收益，仅保留可选。两项默认仍
关闭，组合不优于单独 A。Nsight 确认 halo 调用及搬运字节数不变，
不能据此宣称完成反应流通信瓶颈优化或 CUDA-aware 生产准入。

下一步针对剩余 halo 主机中转与 rank 分阶段到达不均做独立评估。
本轮不改 MPI 后端或同步策略，不替代长时反应 SBLI 物理验证。
详细范围、轮间波动、首版负结果和证据目录见性能优化计划。

## 2026-09-25 AIR5 重复通量重构优化完成

`symmetric_species` 的 GPU base/face/apply 已按实际需要裁剪高低阶和
方向通量计算，复用原公式，不增全域缓存。FP64、显式同步、保正投影、
双温能量相容性、MPI 最终共享面和默认 `full_state` 均保持不变。

同一 Mach4 检查点的交错 A/B 三轮完整步计时：

| 配置 | 原基线 s/步 | 优化后 s/步 | 耗时降低 |
| --- | ---: | ---: | ---: |
| 单卡 | 1.340148 | 1.125848 | 15.99% |
| 双卡 x-slab | 0.848744 | 0.736944 | 13.17% |

双卡强扩展效率从 78.95% 变为 76.39%，不宣称通信效率改善。
未新增持久工作区；NCU 显示 face/apply 溢出请求显著减少，未测总峰值
显存。主要收益来自去掉 apply/base 的多余重构，而非取消检查或降低精度。

数值门槛：x/y/z 四类压力矩阵 36 次运行、1080 对逐位共享面通过，
另有默认路径三次对照、激波管 NP2 五步、Mach4 NP1/NP2 场比较和
非周期边界收支通过。本轮共 27 份 GPU rank memcheck 零错误，
36 项相关检查器测试通过。Mach4 最大阶段差 8.077936e-28，限于痕量
N/O/NO；满足原容差但不宣称全场逐位一致。冻结激波管的传感器实际
标记了界面，不代表真实反应 SBLI 已物理验收。

保留局部优化，不提升默认数值选项。下一阶段另设同状态物性/原始量
恢复复用与全局检查、通信编排目标，不放宽化学或反演容差。
完整证据及复现命令见 [性能计划](ASTR_GPU_PERFORMANCE_OPTIMIZATION_PLAN.md)。
实测冻结二进制 `b2ff45ee...39f3b1`，最终重链接 `e1eb9579...4c57e5`
已补跑双卡二十份阶段场逐位一致和 memcheck，但不替代原计时版本。计时证据
`tests/gpu_validation/out/air5_flux_opt_ab_20260925/summary.json`。

## 2026-09-25 AIR5 本地性能诊断完成

在 `7bf99c5` 数值基线之上仅新增默认关闭的公共完整步计时，保持
`symmetric_species + layered`、FP64、显式同步。复用 Mach4 无入射激波
平板检查点，64x512x8 节点、dt=2 ns。每组独立运行三轮，预热一步，
计时三步，不在窗口内写大型场文件，完整计入化学、输运、边界和 MPI。

| 配置 | 完整步中位数，s | 相对 CPU NP1 |
| --- | ---: | ---: |
| CPU NP1 | 80.098913 | 1 |
| RTX 4000 Ada 单卡 | 1.336195 | 59.9455 |
| 双卡 NP2 x-slab | 0.848752 | 94.3726 |

双卡相对单卡加速 1.57431，强扩展效率 78.7153%。CPU 基线仅为
Xeon Gold 6248R 单 MPI rank，不能表述为相对整台 CPU 节点加速。
本轮仍采用一般 halo 的 pageable host-staged 基线，不代表最优通信配置。

Nsight Systems NP1/NP2 和 Nsight Compute 前三个热点核均已成功采集。
单卡界面限制、化学积分、通量应用核分别占 kernel 时间约
34.91%、25.26%、10.32%。前两核的 FP64 管线利用率约 82% / 85%，
应用核及界面核存在寄存器溢出。双卡 MPI 暴露时间约 65--68 ms/步/rank，
Allreduce 等待多于 Sendrecv，且不同计算阶段的 rank 耗时不均。
不能将双卡效率损失全部解释为 halo 网络通信，也不能将同步 API 等待
全部算作可消除开销。

CPU 计时开/关十份阶段场及 GPU NP2 新旧二进制二十份场逐位一致；
CPU/GPU NP1 和 GPU NP1/NP2 满足原容差，状态保正/物性域门槛通过，
23 项相关检查器测试通过。结果不关闭真实激波反应流、长时物理收敛
或 A800 生产性能门槛，不提升默认限制器。

下一步优先评估按方向/高低阶拆分通量重构，减少重复 FP64 运算和大局部
数组；其次分析双温原始量恢复与化学计算复用，最后按证据优化全局检查
和通信。所有候选需要独立目标及相同数值门槛，本轮未实施算法优化。
详细数据、采集口径和风险见 [性能计划](ASTR_GPU_PERFORMANCE_OPTIMIZATION_PLAN.md)。
合并计时证据：`tests/gpu_validation/out/air5_perf_diagnosis_20260925/timing_summary.json`。
