...
2026-09-07，用户要求设定并执行完整 OpenSBLI 仿真 goal。当前工作状态：已获度量一致 eps 试验授权，继续验证，尚未完成。
...
| G2 边界 | CPU/GPU 可选参考边界，顶层 CMake 构建和独立边界/halo/RK 测试 | 物性及基础算子通过，主循环/角点/MPI 集成待做 |
| G3 一致性 | NP=1、NP=2/4，同相位逐场/统计 atol=rtol=1e-10，z均匀性 | 待做 |
| G4 时间 | 参考网格积分至 t=13000，继续到 t=26000检查 | 待做 |
...
下一验收关口仍为生产模式准入、GPU 阶段调度、初始化/RK 接线和 NP=1/2/4
短步同相位比较。当前未启动完整参考网格长时间仿真，完整 goal 未完成。
...
下一步仍为生产模式准入和初始化/RK 集成，不能据此启动无准入保护的长跑。
