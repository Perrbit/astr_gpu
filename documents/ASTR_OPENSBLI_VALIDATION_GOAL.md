...
参考边界已接入 CPU/GPU 生产主循环，restart 验证通过，NP=2 长跑已到
`t=13000`。原定摩阻、壁压和分离长度时间门槛通过；网格/时间步矩阵、热流
一致性、长期 z 均匀性和完整外部物理验证尚未完成。
...
| G4 时间 | 参考网格积分至 t=13000，继续到 t=26000检查 | 部分通过：t=13000 到达且原定三项时间门槛通过；t=26000 未执行 |
| G5 网格 | 至少三档网格，固定物理域与物性，时间收敛后比较 | 待做 |
| G6 外部验证 | 原始参考壁压/Cf/分离区/场结构对比，报告与可复现脚本 | 部分通过：壁压/Cf/激波位置/分离区通过；热流和 z 均匀性未闭合 |
...
续算已到 `nstep=325000, t=13000.000000098078`，不再属于“长跑尚未到达目标
时间”。采用每个 x 位置的局部物理 y 节点和六点多项式壁面导数后，摩阻、
壁压和分离区与公开 OpenSBLI 场吻合：摩阻和壁压峰值尺度误差分别为
`0.7413%` 和 `0.1511%`，分离长度误差为 `0.0603%`，最大壁压梯度位置均为
`x=130.26315789473685`。

完整门槛仍未通过。最终两个 checkpoint `t=12800/13000` 之间，摩阻、壁压
和分离长度变化均低于本文件冻结的 `1%`，所以 G4 原定时间门槛通过。同期
壁面热流峰值尺度变化为 `2.488%`，相对公开场重构热流的峰值尺度误差为
`60.6%`；本文件此前没有冻结热流时间阈值，不能事后把 `1%` 自动扩展给热流。
周期首尾平面逐场相等，但唯一展向平面相对展向均值的最大偏差为 `4.397e-4`，
长期 z 均匀容差也尚未冻结。G5 三网格两时间步仍待完成，G6 仅壁压、摩阻、
激波位置和分离区子集通过。
