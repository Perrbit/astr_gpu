...
状态：Phase C0 至 C4 已完成；Phase C5 第 1 至 5 项、第 6 项 A0 短时门槛及 C5-6B 的 `q(1:11)` 激波格式核心/周期激波管 NP=1、三种 NP=2 slab 和 100 步接缝门槛已完成，A1 独立双温物理验证和 C5-6B 正常激波/SBLI 待推进
日期：2026-09-20
...
同一两步 HBL 在 `lfilter=t` 时仍以 `z` 挤出缩放误差约 `1.4847e-5` 失败。修改前
保留的旧可执行文件给出约 `1.484665e-5`，证明该失败不是移除经验裕量造成，而是
当前反应流十阶滤波路径已有的挤出不变量缺陷。在该缺陷单独定位并通过严格门槛前，
`lfilter=t` 不晋级为已验证的反应流能力，也不继续长步和性能测试。
...
C5-6A0 短时门槛及 C5-6B-B0 的 `q(1:11)` 数值核心/周期激波管门槛已通过。
独立有限速率 A1-R2 数值基线已通过；当前继续完成 A1-R3 的 ASTR CPU/GPU
同站位物理验证并关闭 C5-6A1，随后推进 C5-6B 正常激波、物理边界、多 rank 和
有限速率 SBLI，之后才进入 C6 性能生产验收。任何阶段未通过本阶段门槛时停止晋级并
