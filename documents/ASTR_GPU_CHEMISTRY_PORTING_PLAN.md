...
frozen-air5 Dorodnitsyn BVP 是权威单温参考，R2/R3 采用全局 `x-y` 质量流函数
配点路线，不再要求逐站后向 Euler 候选承担参考职责。BVP 不调用 ASTR
...
了当前 HBL 的痕量对流/扩散正性数值阻塞，但不代替长时稳定性、网格/时间步收敛
和独立双温平板物理比较。
...
仍待人工确认的后续化学数值决策为 `q(1:11)` 局部 Lax--Friedrichs/MP7 激波
格式；在该格式确认和独立激波门槛通过前不进入有限速率空气 SBLI。
