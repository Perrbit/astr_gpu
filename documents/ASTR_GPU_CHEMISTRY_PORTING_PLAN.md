...
状态：Phase C0 至 C4 已完成；Phase C5 第 1 至 5 项已完成，第 6 项 A0 已接入但停在非催化壁面组分通量闭合
日期：2026-09-14
...
主循环中常驻推进。Strang 耦合、反应边界/重启合同和反应流物理算例仍分别属于
C5 至 C6。
...
固定 air5 的完整总能量和 `q(11)=Ev` 语义。A0 的离散壁面通量闭合、A1 独立
双温参考和 B 阶段激波格式未通过前，不得通过人工目标场或放宽误差门槛宣称
C5 完成。
...
质量分数。该失败不得通过组分裁剪、归一化或未审批的一阶回退掩盖，C5-6A0
仍为未通过。

下一步需要人工确定离散非催化壁面合同。推荐方案 A：壁面热力学状态采用第一
内点的正质量分数，同时在 Cartesian y 法向组分扩散算子中显式置零法向质量分数
梯度，使 `J_s,n=0`。由物种扩散携带的焓和振动能通量随之为零，温度与 `Tv`
梯度产生的导热通量继续保留。CPU 在通量构造前覆盖壁面节点
`dspc(:,0,:,:,2)`；GPU 在现有组分梯度 kernel 内仅对下壁面拥有者的 `j=0`
物理 y 分量执行同一操作，并保持 kernel 后显式同步。未来 CURVE 路径必须改用
几何法向投影。方案 B 是正性约束的高阶外推或投影，实质上引入限制器，
需要独立精度分析。方案 C 只复制第一内点而不修改通量算子，既不严格满足当前
单边离散零梯度，又发生局部降阶，不建议采用。

仍待人工确认的后续点包括方案 A 的离散通量闭合，以及 `q(1:11)` 局部
Lax--Friedrichs/MP7 激波格式。在壁面闭合确认前不继续 A0 运行；在激波格式确认
前不进入 C5-6B。
...
本计划的首阶段技术决策已经全部确认。Phase C0 至 C4 已通过各自门槛，当前从
Phase C5 开始推进 Strang 反应流耦合，并继续按 C5 至 C6 逐级推进。任何阶段未通过
本阶段门槛时停止晋级并直接报告逻辑、模型或实现问题，不得通过跳过测试、修复
状态或放宽门槛继续下游算例。
