...
- Static single-block curvilinear-grid validation has reached the C6 x-wall gate:
  ...
  - C6 establishes numerical equivalence and algebraic wall invariants for this static x-wavy single-block slice. It does not validate curved-wall shear stress, heat flux, wall-unit scaling, or any other physical wall diagnostic. CPU y/z symmetry ambiguities, moving grids, and multi-block grids remain outside scope.
...
- Phase C boundary expansion has started with CPU-compatible symmetry:
  ...
  - The y/z GPU implementations retain the CPU Cartesian behavior used by the original TGV-template validation: zero the coordinate-normal velocity component, extrapolate tangential primitive fields, rebuild `q`, and zero physical-boundary `qrhs`.
  - The x implementation now supports a static curvilinear physical face. Resident `bnorm_i0_d/bnorm_im_d` planes remove the extrapolated velocity component normal to each local physical face, matching CPU `symmetry(ndir=1/2)`.
  ...
  - Curvilinear y/z symmetry remains closed because the current CPU y and k-min branches do not consistently apply their computed geometric normals. No CPU correction or ambiguous boundary behavior is ported without an explicit decision.
