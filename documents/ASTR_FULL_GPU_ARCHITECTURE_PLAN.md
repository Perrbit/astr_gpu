...
- The current `256^3`, NP=1, FP64, explicit-sync TGV median is
  `0.580044424 s/RK`, reduced by `23.964%` from the original
  `0.762855769 s/RK` baseline. A 12-kernel NCU matrix attributes the latest
  retained gain to reuse of the CPU `num1d60` compile-time constant across
  GPU sixth-order convection, diffusion, and `gradcal` derivatives.
