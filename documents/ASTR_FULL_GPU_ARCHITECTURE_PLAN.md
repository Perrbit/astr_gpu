...
- Static single-block curvilinear-grid validation has completed the C21 aggregate closure, with C15 retaining the large-grid performance evidence:
  ...
  - C16 generalizes the C6 `bctype=41` validation harness from the x-wavy pair to x-, y-, and z-wavy physical wall pairs. The reusable 15-entry matrix covers NP=1 convection isolation, NP=1 five-step explicit filter plus diffusion, physical-axis NP=2 five-step slabs, NP=4 planes, and NP=8 `2x2x2` for every physical axis.
  - All C16 entries pass with maximum full-field error `q5=6.2527760746888816e-13`, maximum statistic error `kenergy=4.9640846988552312e-14`, and maximum wall-invariant error `1.1368683772161603e-13`. Wall velocity and momentum residuals are zero, all analytic mappings have `J=1`, and targeted y/z NP=1 Compute Sanitizer runs report zero errors.
  - C16 closes numerical equivalence for the zero-blowing static single-block six-face isothermal no-slip subset. No geometric velocity projection is needed because the prescribed velocity is zero. CPU pressure extrapolation remains along the computational coordinate line. The optional y-min `wallbs.dat` route is closed separately by C19.
  - C17 closes CPU-compatible `bctype=50` zero extrapolation on x/y/z wavy face pairs. Its 15-entry NP=1/2/4/8 matrix passes with maximum field/statistic/boundary residuals `3.9790393202565610e-13`, `4.9960036108132044e-14`, and `7.6170181273482740e-12`; targeted y/z memchecks are clean. The invariant oracle preserves CPU's directional asymmetry: x reconstructs density from extrapolated pressure/temperature, while y/z reconstruct temperature from extrapolated pressure/density.
  - C18 closes the zero-blowing CPU-defined `bctype=42` subset on x/y wavy face pairs. Ten supported NP=1/2/4/8 entries and the required z reject pass; maximum field/statistic/wall residuals are `3.6948222259525210e-13`, `4.9071857688431919e-14`, and `5.1585402616183273e-12`, and x/y memchecks are clean. CPU defines no z branch. The optional y-min `wallbs.dat` route is closed separately by C19.
  - C19 changes CPU and GPU y-face `411/421` slip sections to extrapolate the full velocity vector and project it onto the local discrete tangent plane. `411` no-slip sections remain zero; `421` lower-y combines tangent projection with prescribed normal blowing and upper-y remains tangent-only. No absent x/z CPU branches are added.
...
- The first dedicated periodic TGV performance closure is complete on the RTX 4000 Ada workstation. For FP64 `256^3`, NP=1, `643e/643e`, tenth-order explicit filtering, and diffusion, five-run median complete-RK time decreases from `0.762855769 s` to `0.682327627 s` (`10.556%` time reduction, `1.11802x` speedup). Run spread is `1.751%`, sampled memory growth is `1.265%`, NP=1 and NP=2 x-slab 10-step field/statistic comparisons pass at `1e-10`, representative memcheck reports zero errors, and the RK trace contains no forbidden transfer at or above 64 KiB. The retained changes preserve direction-specific block shapes and every explicit kernel synchronization. Removing redundant synchronizations while retaining numerical consistency is reserved for a separate performance Goal. A separate A800 rerun is required before making an A800 performance claim; see `documents/ASTR_TGV_PERFORMANCE_OPTIMIZATION_REPORT.md`.
...
- Phase E-G wall-family expansion has added CPU-compatible Cartesian slices:
  ...
  - `bctype=411` slip-isothermal walls are supported for y only. x/z are intentionally rejected because CPU `slipisotwall` implements only `ndir=3/4`. C19 upgrades y slip sections to geometric tangent projection and supports signed physical-normal blowing on the lower no-slip section.
  - `bctype=421` slip-adiabatic walls are supported for y only. x/z are intentionally rejected because CPU `slipadibwall` implements only `ndir=3/4`. C19 upgrades both y slip faces to geometric tangent projection and permits signed physical-normal blowing on lower y.
  - These phases plus C19 validate the y-face curved slip/blowing contract. They do not add x/z `411/421`, z `42`, turbulence wall models, species, chemistry, or compact schemes.
