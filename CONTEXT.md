...
**Optional single-work-array filter path**:
The validated full-variable ping-pong explicit-filter implementation remains the default and fallback. A future memory-saving option may reuse one scalar 3-D work array across `numq` conservative variables, reducing temporary filter storage from one full `numq` field set to one scalar field. This option must preserve the same tenth-order explicit center-filter coefficients, CPU `filterq` timing, dataswap-compatible `hm` halo semantics, and old-GPU/full-ping-pong versus new-GPU/work-array validation. It is not a direct in-place overwrite scheme and not a replacement for the current ping-pong path.
_Avoid_: Removing the validated ping-pong path, claiming the work-array path is faster before profiling, doing unsafe in-place stencil overwrite across GPU blocks or MPI faces
...
**CPU bug decision gate**:
When GPU porting exposes a clear CPU design bug or numerically invalid CPU behavior, the GPU implementation must not automatically reproduce that behavior as the long-term semantics. The issue should be reported to the user for an explicit human decision first: either fix the CPU oracle/common path, temporarily compatibility-match the CPU behavior behind a documented validation gate, or reject/defer that case. This applies especially to stale halo usage, missing boundary closures, invalid active ranges, non-taken expressions that still evaluate invalid arithmetic, and boundary-condition branches whose active code contradicts the intended physics.
_Avoid_: Silently porting known CPU bugs, treating CPU/GPU agreement with an invalid CPU oracle as correctness, hiding CPU fixes inside GPU-only logic
...
**Phase S2-A0 oblique-shock HBL compatibility gate**:
The first S2 slice is a controlled CPU/GPU equivalence gate, not a resolved physical SBLI validation. `tests/gpu_validation/run_s2_hbl_oblique_shock_compare.sh` keeps the S1 Mach-5 Sutherland flat-plate contract, initializes from `ninit=3`, and overlays an analytic perfect-gas oblique shock on the x-varying compressible Blasius HDF field. The overlay updates `rho,T,u,v` consistently so CPU pressure reconstruction from `rho*T` is closed. The `64x64x8` one- and two-step smoke gates pass at `1e-10`; the two-step smoke has `q5 L_inf=8.8818e-16`. The default `192x192x8` two-step gate passes for NP=1, NP=2 x/y/z, NP=4 xy/xz/yz, and NP=8 `2x2x2`; z-decomposed cases use `KM=16`, all MPI runs retain `q5 L_inf=1.3323e-15`, and `run_s2_hbl_oblique_shock_mpirank_matrix.sh` captures the matrix plus NP=1/NP=8 `MAXSTEP=20` long subset. The long subset passes with `q5 L_inf=3.3307e-15` and max stats diff `1.2062e-12`; optional `RUN_STRESS=t MAXSTEP_STRESS=100` passes NP=1 and NP=8 with `q5 L_inf=5.7732e-15` and max stats diff `1.2217e-12`. This gate exposed a GPU `bctype=21` outlet bug: CPU extrapolates sound speed directly, while GPU previously computed `sqrt(extrapolate(T))/Mach`; `outflow_x_kernel` now matches CPU `extrapolate(sos(T1),sos(T2))`.
_Avoid_: Calling S2-A0 a physical SBLI validation, enabling RANS/LES, claiming characteristic farfield/inflow standardization, or treating the 64-grid smoke as mesh/time convergence

**Phase S2-B1 inlet-sustained compressed-layer gate**:
`inletprofile` now accepts a backward-compatible optional fifth `pressure` column when the first profile line contains `pressure=provided`. With `density=provided pressure=provided`, it preserves file density and pressure but stops if `rho*T/(gamma*Mach^2)` is inconsistent with the pressure column. `tests/gpu_validation/run_s2_hbl_inlet_sustained_shock_compare.sh` reuses the S2-A0 driver with `PROFILE_OBLIQUE_SHOCK=t`, `PROFILE_PRESSURE_MODE=provided`, and a shock line entering at the inlet, so the upper inlet profile continuously injects the compressed post-shock state. This is still a boundary-forced compatibility gate, not full physical SBLI. The `64x64x8` two-step smoke passes with `q5 L_inf=8.8818e-16`; the default `192x192x8` NP=1 two-step gate passes with `q5 L_inf=1.3323e-15` and max stats diff `7.8382e-13`. The two-step `192x192x8` core MPI matrix also passes for NP=2 x/y/z, NP=4 xy/xz/yz, and NP=8 `2x2x2`; z-decomposed cases use `KM=16`, all runs retain `q5 L_inf=1.3323e-15`, and the largest statistics difference is the NP=2 z-slab `massflux=1.2244e-12`.
_Avoid_: Treating sustained profile compression as a characteristic shock generator, claiming mesh/time convergence, or applying pressure-provided profiles without EOS consistency checks

**Phase S2-C characteristic SBLI design**:
The next SBLI architecture step is documented in `documents/ASTR_S2_SBLI_CHARACTERISTIC_DESIGN.md`. Maintainer feedback is that farfield/NSCBC behavior varies strongly by case, so the near-term GPU route should not be blocked by the CPU `farfield_nscbc` unconditional acoustic overwrite question. Keep the simplest already validated boundary contract for the first common-path gate, then couple the existing Ducros sensor and selective-Roe reconstruction into the HBL/SBLI setup. S2-C1 characteristic profile inflow and S2-C2 upper-y farfield remain later case-specific branches; the immediate implementation target is S2-C3, using S2-B1/simple boundaries plus rankwise sensor/mask and field/statistics validation. The physical SBLI target rule for future true characteristic boundaries remains incoming-wave-only correction, but it is no longer the next implementation dependency.
_Avoid_: Letting farfield/NSCBC policy block the common shock-format path, combining characteristic inflow, upper farfield, selective Roe, outlet NSCBC, and sponge in one first gate, treating simple-boundary S2-C3 as full physical SBLI validation

**Phase S2-C3 selective-Roe HBL status**:
S2-C3 now has a CUDA path for `flowtype=bl`, `543e/643e`, `lchardecomp=t`, `lfilter=f`, `diffterm=f`, simple `11,prof/21/41/51` boundaries, and pressure-provided sustained compressed inlet profiles. GPU adds xy-physical characteristic flux/RHS kernels, y-physical MP7/SUW3/MP5 degradation inside the characteristic reconstruction path, and CPU-compatible `npdci/npdcj` shock-sensor boundary clamping. `run_s2_hbl_selective_roe_s2c3_compare.sh` is the regression entry. The enabled topology set is NP=1, all NP=2 slabs, all NP=4 two-axis slabs (`2x2x1`, `2x1x2`, `1x2x2`), and NP=8 `2x2x2`. The `64x64x16` z-decomposed gates keep local `km=8>=hm`; every one- and three-step CPU/GPU field comparison passes at `1e-10`, with final `q5 L_inf=8.88e-16` for the combined NP=4 and NP=8 gates. The `64x64x8` NP=1 and `64x64x16` NP=8 `2x2x2` 20-step gates pass with final `q5 L_inf=2.00e-15` and `1.33e-15`; their 100-step gates pass with `3.33e-15` and `4.88e-15`. Root cause: the characteristic vector Steger-Warming split periodized `jacob_d` indices even when its stencil read exchanged physical/MPI geometry halos; its scalar counterpart already used direct halo indices. Direct halo geometry access restores CPU/GPU y-interface flux agreement.
_Avoid_: Treating this topology-complete simple-boundary equivalence matrix as physical SBLI validation, treating full physical-y sensor dumps as a clean oracle near CPU halo-history-dependent physical boundaries, enabling diffusion/filter/NSCBC/farfield/sponge in this gate

**Phase S2-C3-S x-max sponge status**:
The existing device-resident x-max `layer` sponge is now coupled to the C3 simple-boundary selective-Roe path under a separate, narrow capability predicate. It is intentionally not S2-C4/C5: it retains `11,prof/21/41/51`, avoids characteristic inlet/farfield policy, and only permits positive `spg_im` with all other sponge extents zero. GPU preserves CPU ordering after every RK substage: q halo exchange, two-kernel `qwork_d` sponge ping-pong, explicit synchronization, then primitive refresh. `run_s2_hbl_selective_roe_s2c3_sponge_compare.sh` validates `SPONGE_IM=16`; NP=1 `64x64x8` and NP=8 `2x2x2` `64x64x16` both pass 1, 20, and 100 steps at `1e-10`. Final 100-step `q5 L_inf` is `7.55e-15` for NP=1 and `8.66e-15` for NP=8.
_Avoid_: Claiming characteristic-boundary or physical-SBLI behavior, enabling y/z/circular sponge, or allowing unvalidated NP=2/4 sponge topology combinations

**Shock-format readiness gate**:
S0-A1 is a non-reacting, no-wall shock-format gate that isolates explicit upwind reconstruction, flux splitting, and shock-format RHS behavior before opening sponge, inlet/outlet, high-speed wall, or full SBLI coupling. It uses a forced 3D extruded Sod case via `sodini`: x contains the discontinuity and y/z are uniform thin directions. `examples/sod/datin/input.sod` is not trusted as-is because it declares `flowtype=shuosher`, uses a 1D grid, compact upwind, and characteristic decomposition. The controlled porting-equivalence gate uses `GRID=200,8,8`, periodic x/y/z, `deltat=5.d-4`, `maxstep=20`, `conschm='543e'`, `difschm='643e'`, `lchardecomp=.false.`, `diffterm=f`, and `lfilter=f`. S0-A1 (`recon_schem=-1`), S0-A2 (`recon_schem=1`, WENO7), and S0-A3 (`recon_schem=3`, MP7) are explicitly single rank. All three pass 20-step CPU/GPU statistics and fields at `1e-10`; WENO7 and MP7 share generic explicit-reconstruction kernels and one scalar `flux_work_d`. The separate `GRID=800,5,5`, `deltat=5.d-4`, `maxstep=400` exact-solution gate passes unchanged finite thresholds for both WENO7 and MP7. WENO7 measured maximum smooth `L1/L2/Linf=7.1864e-4/4.0087e-3/4.6338e-2`, bound violation `1.3889e-2`, contact/shock thickness `2.9780/2.1989` cells, and position error `0.7527` cells. MP7 measured `7.2439e-4/4.1951e-3/4.8797e-2`, `8.5021e-3`, `2.9496/1.6939` cells, and `0.7272` cells. MP7 is sharper on this shock but is not uniformly lower-error in smooth regions. S0-A4 is a single-rank sensor-only gate using forced-3D Shu-Osher because initial Sod velocity produces an all-zero Ducros factor. S0-A5 extends it to `NP=2 TOPOLOGY=2,1,1`: `ASTR_SHUOSHER_SHOCK_X=0.d0` places the test jump beside the x-slab interface, and raw `ssf` is exchanged through the generic `hm`-layer field transport before mask expansion. The merged CPU/GPU raw field passes at `L_inf=1.1102e-16`, the 1944-node mask matches exactly, and two-rank memcheck passes with MPI CUDA probing disabled. S0-A6 then consumes the same mask at `NP=1`: sensor-active interfaces use Roe projection, MP7 characteristic reconstruction, and conservative back projection, while smooth interfaces retain physical-space MP7. The one-step field gate gives `q5 L_inf=4.4409e-16`, the three-step gate `1.4211e-14`, and memcheck passes. Its prescribed 512-thread x kernel uses an NVHPC 128-register cap. Caching one five-component Steger-Warming split for each original positive and negative stencil point reduces x/y/z characteristic flux means by `1.93x/2.52x/2.74x` without a global field, kernel, or new synchronization; it remains a periodic selective-Roe result only. S0-A7/A8/A9 validate x/y/z two-rank characteristic decompositions, and S0-A10 validates the combined `2x2x2` topology with rankwise y/z mask ownership. S0-B owns outlet, sponge, NSCBC, and other open-boundary treatment. Compact upwind, physical-boundary MP5 fallback, MP-LD coupling, diffusion, and filtering remain unsupported in the completed shock slices.
_Avoid_: Starting Phase S with full SBLI, diagnosing wall/sponge/stability issues before the shock format itself is validated, using the current `input.sod` without correcting/overriding its flowtype, reopening compact schemes in the shock gate, treating CPU/GPU agreement alone as exact-solution accuracy, treating MP7's sharper shock as uniformly better accuracy, treating physical-boundary MP5 or multi-rank support beyond the documented S0-A5 x-slab gate as completed, using a 3-D validation dimension smaller than `hm=5`, evaluating the periodic-boundary Riemann problem as part of the central Sod oracle, enabling diffusion/filtering in these gates, describing periodic `recon_schem=1` as WENO5, treating the S0-A4/A5 validation side path as characteristic reconstruction
...
_Avoid_: Claiming wall42 z support, attributing later C19 y-min blowing evidence to x walls, claiming turbulence wall model support, or general wall-boundary support from the Phase E Cartesian slice

**Phase F wall411 slice**:
GPU `bctype=411` support is limited to y slip-nonslip isothermal wall slices. CPU `slipisotwall` implements only `ndir=3/4`; x/z wall411 have no CPU branch and are intentionally rejected by `BC_KIND=slipisotwall ZERO_AXIS=x/z`. C19 supersedes the original Cartesian component clamp: the lower `x <= xslip` section now extrapolates all velocity components and projects onto the physical tangent plane, while the lower no-slip section remains zero unless physical-normal blowing is configured. The upper wall remains no-slip isothermal. Single-rank y, NP=2 physical y, and NP=2 transverse x/z slab 3-5 step `LFILTER=t,DIFFTERM=t` statistics plus field comparisons pass at `GRID=32,32,32`; C19 adds y-wavy NP=1/2/4/8 coverage.
_Avoid_: Claiming wall411 x/z support, turbulence wall model support, species support, or extending the y-face C19 contract to arbitrary wall combinations

**Phase G wall421 slice**:
GPU `bctype=421` support is limited to y slip-adiabatic wall slices. CPU `slipadibwall` implements only `ndir=3/4`; x/z wall421 have no CPU branch and are intentionally rejected by `BC_KIND=slipadibwall ZERO_AXIS=x/z`. C19 supersedes the original Cartesian component assignments: both y faces extrapolate all velocity components and project onto the physical tangent plane; optional y-min blowing is added along the normal. The historical commented `xslip` split remains inactive. Single-rank y, NP=2 physical y, and NP=2 transverse x/z slab 3-5 step `LFILTER=t,DIFFTERM=t` statistics plus field comparisons pass at `GRID=32,32,32`; C19 adds y-wavy NP=1/2/4/8 coverage.
_Avoid_: Claiming wall421 x/z support, active xslip split behavior, turbulence wall model support, species support, or extending the y-face C19 contract to arbitrary wall combinations
...
The GPU runtime gate is increasingly capability-based rather than a hard-coded `flowtype` whitelist. The broad shared periodic path still requires 3D, `numq=5`, no species, no modal equations, no turbulence model, `rk3`, explicit `643e,643e`, homogeneous x/y/z, consistent MPI topology, and positive Jacobian; this covers TGV, 3D extruded `2dvort`, and HIT. Later validated gates add specific non-periodic boundaries, wall slices, LDC, RTI, and source dispatch. S0-A1/A2/A3 are separate controlled single-rank forced-3D Sod capabilities for `conschm='543e'`; S0-A4 is the validation-only single-rank forced-3D Shu-Osher sensor capability, S0-A5 is its `NP=2 TOPOLOGY=2,1,1` raw-sensor halo extension, S0-A6 is a separate `NP=1` Shu-Osher Roe/MP7 characteristic capability, and S0-A7/A8/A9 extend it to x/y/z two-rank slabs while S0-A10 validates `2x2x2`. All S0-A4 through S0-A10 gates require `ASTR_SHOCK_SENSOR_DUMP`; every decomposed direction must retain `local_n>=hm`, and y/z masks require rankwise CPU/GPU comparison because CPU owns duplicate active-interface masks locally.
_Avoid_: Treating `flowtype` names as the support contract, accidentally accepting compact or unsupported non-periodic cases because their name is known, extending a controlled S0 capability beyond its validated contract, enabling Shu-Osher GPU execution without the explicit S0-A4/S0-A5 validation switch
...
GPU `bctype=60` symmetry supports exactly one physical x/y/z direction, with the other two directions homogeneous periodic. On a curvilinear face, CPU and GPU extrapolate the full velocity vector and apply `u_t=u^*-(u^* dot n)n`; all six CPU-generated `bnorm_i0/im,j0/jm,k0/km` planes are resident on the device. This replaces the former y-face and k-min Cartesian component clamps after explicit approval. CPU `geom` also now forms `bnorm_jm` from the correct `j=jm` metric plane. True physical faces are gated by `MPI_PROC_NULL`, internal interfaces use exchanged halos, q is rebuilt on device, and physical-boundary qrhs is zeroed. CURVE-C13 passes x/y/z wavy-face tests at NP=1/2/4/8; the maximum discrete-normal no-penetration residual is `7.2316284904783146e-17` and x/y/z memchecks report zero errors.
...
**CURVE-C10 viscous selective-Roe status**:
The static single-block curvilinear Mach 5 path now combines the C9 Ducros/selective Roe-characteristic MP7 convection with sixth-order explicit diffusion and Sutherland viscosity under the simple `11/21,41/51`, z-periodic contract. The separate `gpu_s2_hbl_selective_roe_diffusion_supported()` capability preserves the original inviscid C9 predicate. The C10 matrix passes NP=1 isolation/20 steps, all NP=2 slabs, all NP=4 planes, and NP=8 `2x2x2`; separate NP=1 and NP=8 100-step gates pass. Maximum observed field/statistic errors are `u1=7.6039174956576971e-13` and `massflux=5.0892623448817176e-13`; one-step sensor masks match exactly with 7659 nodes, wall invariants stay below `1.8e-15`, Jacobians remain positive, and NP=1 Compute Sanitizer reports zero errors. A dedicated no-checkpoint C10 Nsight trace contains no H2D/D2H operation at or above 64 KiB after the first characteristic x-RHS kernel; H2D is `3 x 176 B` and D2H is `3 x 6144 B`.
_Avoid_: Calling C10 physical SBLI validation, enabling global explicit filtering in the shock region without a separate numerical-policy decision, or broadening the capability to NSCBC/sponge/general curved faces without independent gates

**CURVE-C11 full-domain filter negative gate**:
The first scheme-A probe retained the CPU full-domain explicit 10th-order filter on the sharp C9/C10 oblique-shock input. At `64x64x8`, NP=1, `lfilter=t,diffterm=f`, the CPU reference became non-finite after the first complete RK step and `crashcheck` stopped at step 1. The same-phase snapshot contained non-finite values in 22529 density entries, 22160 entries in each velocity component, 22412 pressure entries, and 22104 temperature entries out of 38025 nodes. No C11 GPU capability or driver is retained. Global centered-filter overshoot across the discontinuity, potentially amplified by filtered-`q`/pre-filter-primitive timing, is a hypothesis rather than an isolated root cause.
The approved policy closes this as a negative gate and keeps shock-containing selective-Roe paths at `lfilter=f`.
_Avoid_: Treating GPU-only completion as validation, loosening comparison tolerances when the CPU oracle is non-finite, or reopening filtered selective Roe without a new numerical-policy decision

**CURVE-C12 characteristic-boundary and sponge status**:
The C10 curved viscous selective-Roe path now composes with `bctype=12/21,41/52` and an optional x-max `layer` sponge. Separate diffusion predicates admit NP=1, all NP=2 slabs, all NP=4 planes, and NP=8 `2x2x2`, while rejecting filtering and all unvalidated sponge extents. The C12 matrix, NP=1 and NP=8 100-step gates, NP=1 memcheck, and a three-step no-checkpoint residency trace pass. Largest observed field/statistic errors are `u1=7.6039174956576971e-13` and `massflux=4.9964921089440395e-11`; sensor masks match exactly, wall invariants remain below `1.8e-15`, and Jacobians remain positive. No RK-window H2D/D2H transfer reaches 64 KiB; maximum sizes are 176 B and 6144 B. A missing pre-statistics q snapshot route for the new diffusion predicates was corrected after explicit approval.
_Avoid_: Calling C12 physical SBLI or mesh/time convergence validation, generalizing its characteristic conditions to arbitrary curved faces, enabling y/z/circular sponge, or treating two-GPU oversubscription as scaling evidence

**CURVE-C13 six-face geometric symmetry status**:
The curvilinear symmetry contract now prioritizes physical geometry on every face. CPU and GPU use the same discrete face normal and project the extrapolated velocity into the local tangent plane. Three analytic `J=1` mappings independently curve x, y, and z face pairs. The 12-entry matrix covers NP=1/2/4/8 and gives maximum CPU/GPU field and statistic errors of `8.5265128291212022e-14` and `4.6171400036598698e-14`. NP=1 discrete-normal residuals are below `7.3e-17`; nonzero coordinate-normal components prove that the result is a projection rather than a component clamp. Independent analytic-normal residuals measure boundary-metric truncation and remain below `6.84e-8`. Targeted x/y/z memchecks are clean, and Cartesian Phase C plus C12 regressions pass.
_Avoid_: Extending this claim beyond static single-block `bctype=60`, evaluating projection accuracy only against analytic normals without separating metric truncation, or claiming production scaling from shared-GPU high-rank tests

**CURVE-C14 physical HBL refinement status**:
The C7 nonorthogonal Mach 5 laminar boundary layer now has a matched-time physical-consistency matrix on `96x96x8`, `144x144x8`, and `192x192x8` grids with `deltat=1e-5` and `5e-6`. Physical gradients are reconstructed from computational derivatives and the mapping before evaluating geometric skin friction and heat flux. Both near-recovery hot-wall and cold-wall families pass all CPU/GPU, wall, positive-J, spatial-refinement, and time-step-separation gates. The largest field/statistic errors are `1.4166445794216997e-13` and `7.616129948928574e-14`. Cold-wall `Cf/qw` spatial-delta ratios are `0.4964/0.5943`; hot-wall `Cf` is `0.4697`; all sampled profile ratios are `0.4355-0.4560`. Hot-wall heat flux is informational because the Blasius reference crosses zero inside an unresolved near-wall interval; cold-wall heat flux is the required thermal diagnostic.
_Avoid_: Calling this a steady DNS, experimental, or SBLI validation; accepting near-zero relative heat-flux errors as well-conditioned evidence; comparing wall derivatives only in computational coordinates

**CURVE-C15 performance and residency closure**:
The full C10 `256x256x256` trace initially exposed a `3,145,728 B` D2H statistics transfer from returning `65536 x 6` block partials. `gpu_s1_bl_scalars()` now performs a deterministic second device reduction and returns six real(8) scalars (`48 B`). The complete C14 dual-wall matrix still passes, targeted NP=1 memcheck is clean, and the post-change RK trace has no H2D/D2H operation >=64 KiB: H2D is `2 x 176 B`, D2H is `2 x 48 B`. For six RK advances with initialization/output included, repeated NP=1 times are `57.297/57.517/57.712 s`; NP=2 x-slab times are `36.320/36.487/36.548 s`, giving median two-GPU speedup `1.5764x` and efficiency `78.82%`. Peak device-level memory is `9876/5573 MiB`, both configurations reach 100% sampled utilization, and `nvitop` confirms separate GPU binding for the two ranks.
_Avoid_: Reporting this as CPU acceleration, scaling beyond two GPUs, long-time throughput, communication overlap, or GPU HDF5 support

**CURVE-C16 six-face isothermal no-slip status**:
The zero-blowing `bctype=41` curvilinear validation harness covers x-, y-, and z-wavy physical wall pairs with the remaining directions periodic. The 15-entry matrix exercises NP=1 convection isolation, NP=1 five-step filter plus diffusion, physical-axis NP=2 slabs, NP=4 planes, and NP=8 `2x2x2`. Maximum field/statistic/wall errors are `6.2527760746888816e-13`, `4.9640846988552312e-14`, and `1.1368683772161603e-13`; y/z targeted memchecks report zero errors. CPU pressure extrapolation still follows the computational line. The optional y-min physical-normal `wallbs.dat` route is validated separately by C19.
_Avoid_: Calling C16 a geometric-normal pressure closure, attributing C19 blowing evidence to x/z faces, extending it to `bctype=42/411/421/51/52`, or using NP>2 shared-GPU entries as scaling evidence

**CURVE-C17 zero-extrapolation six-face status**:
The `bctype=50` curvilinear harness covers x-, y-, and z-wavy physical pairs at NP=1/2/4/8. The boundary oracle follows CPU direction-specific semantics and reads the CPU complete-RK snapshot: x extrapolates velocity, pressure, and temperature and reconstructs density; y/z extrapolate velocity, pressure, and density and reconstruct temperature. All 15 entries pass with maximum field/statistic/boundary residuals `3.9790393202565610e-13`, `4.9960036108132044e-14`, and `7.6170181273482740e-12`; targeted y/z memchecks are clean.
_Avoid_: Requiring density itself to be linearly extrapolated on x faces, requiring temperature itself to be linearly extrapolated on y/z faces, or comparing boundary invariants at unmatched output phases

**CURVE-C18 adiabatic no-slip status**:
The zero-blowing CPU-defined `bctype=42` subset supports x/y faces only. Ten x/y wavy-face NP=1/2/4/8 entries pass, z is rejected before launch, and maximum field/statistic/wall residuals are `3.6948222259525210e-13`, `4.9071857688431919e-14`, and `5.1585402616183273e-12`. X/y targeted memchecks report zero errors. Pressure and temperature remain computational-line extrapolations. The optional y-min physical-normal `wallbs.dat` route is validated separately by C19.
_Avoid_: Claiming z-face or x-direction wall-blowing `42` support, describing computational-line extrapolation as a geometric-normal Neumann condition, or broadening C18 to slip-wall semantics

**CURVE-C19 y-face slip-wall and wall-blowing status**:
CPU and GPU `bctype=411/421` y-face slip sections now extrapolate all three velocity components and project them with `u_t=u*-(u* dot n)n` onto the local discrete tangent plane. `411` no-slip sections remain zero. `421` lower y combines tangent projection with signed physical-normal blowing; upper y remains tangent-only. The y-min `41/42/411/421` blowing amplitude follows the stored inward normal, and the 10% perturbation uses global i/k indices with a fixed integer hash. The 15-entry y-wavy NP=1/2/4/8 matrix includes negative-amplitude suction and passes with maximum CPU/GPU field, statistic, topology, and normal residuals `8.5265128291212022e-14`, `4.6865289426989420e-14`, `5.6843418860808015e-14`, and `6.0281640790194047e-17`. Five-step filter-plus-diffusion `411/421` regressions, blowing-`421` memcheck, and no-checkpoint residency pass; the RK trace contains no transfer at or above 64 KiB.
_Avoid_: Inventing x/z `411/421` branches, claiming z `42`, treating NP>2 shared-GPU correctness as scaling, or describing computational-line pressure/temperature extrapolation as geometric-normal closure

**CURVE-C20 open-boundary inventory status**:
CPU `bctype=11` now imposes the complete `rho_in/vel_in/prs_in/spc_in` state, validates `tmp_in` against the EOS, and reconstructs temperature; GPU updates all resident primitive and conservative inlet fields consistently. A stale-initial-state gate passes with maximum x-min target error `4.44e-15`, EOS residual `2.22e-16`, and full-field `q5` CPU/GPU error `7.11e-15`; its targeted Compute Sanitizer run reports zero errors with OpenMPI `ob1/self/pt2pt` isolation. Open boundaries remain deliberately restricted: `21/51` require axis-aligned physical faces, `22` is rejected on nonorthogonal grids, and `12/52` retain only the existing Cartesian OpenShock and C12 boundary-layer compositions. One-step Cartesian `22`, C7, and C12 regressions pass after the new checks.
_Avoid_: Treating prescribed `11` as characteristic inflow, claiming arbitrary curved `21/22/51/52`, inferring absent CPU directions, or changing deferred NSCBC relaxation policy under the C20 inventory

**CURVE-C21 aggregate closure status**:
The final aggregate driver rebuilt the CPU/GPU binaries and passed 23 stages covering C0-C20, required rejects, and representative x/y/z memchecks. Across 136 field reports, 128 statistic reports, and 226 boundary-invariant reports, the maxima are `q5=6.2527760746888816e-13`, `massflux=2.5093260802577788e-11`, and `7.6170181273482740e-12`; all 70 explicit finite-field checks pass and the minimum numerical Jacobian is `2.5296638468231652e-7`. The `256^3` no-checkpoint interval contains 221 kernels, zero transfers >=64 KiB, H2D `2 x 176 B`, and D2H `2 x 48 B`. Current rerun medians are `58.490 s` for GPU NP=1 and `38.989 s` for NP=2 x-slab, giving `1.5002x` speedup and `75.01%` two-GPU efficiency.
_Avoid_: Extending this closure beyond static single-block nonreacting explicit schemes, treating the C11 closed-negative filter policy as a passing shock-filter implementation, or claiming arbitrary curved open boundaries, moving/multi-block grids, GPU HDF5, or scaling beyond two GPUs

**P2/P3 performance closure status**:
The local performance campaign is closed. P2 retains only the physical-flux reuse candidate: complete-RK medians for Shu-Osher `256x64x32` and controlled SBLI `256x192x32` improve by `27.234%` and `48.910%` relative to their P2-0 GPU baselines. P3 keeps pageable host-staged blocking as the default, pinned blocking as an option, and pinned overlap only for the fully periodic stored-diffusion interior path. Across the final NP=2 dual-physical-GPU matrix, pinned reduces complete-RK time by `2.829%--23.728%`; paired x/y/z TGV overlap adds `2.616%/3.085%/2.944%` over pinned. Standalone nonblocking was rejected below the 3% gate, and CUDA-aware MPI was deferred after current-stack large-message or sanitizer failures. Every retained path preserves FP64, explicit per-kernel synchronization, halo semantics, and the pageable fallback.
_Avoid_: Relabeling these GPU-internal gains as CPU-to-GPU speedup, extrapolating them to A800 or multi-node systems, enabling periodic-diffusion overlap for physical boundaries or shock sensors, or deleting the portable pageable baseline

**OpenSBLI production and restart status**:
The optional perfect-gas conservative-boundary mode is connected to the CPU/GPU production loop for the 609x255x9 OpenSBLI Katzer laminar-SBLI setup. Fresh initialization requires an x-uniform profile; restart accepts evolved nonuniform physical fields, reconstructs the prescribed inlet ghost density and momentum from `inlet.prof`, and rebuilds the remaining boundary and MPI halos. CPU NP=1 and GPU NP=2 continuous four-step versus two-step-plus-restart checks agree within `1.3322676295501878e-15`; NP=1/2/4 admission, empty-halo restart stages, nstep mismatch rejection, and GPU memcheck pass. A service-isolated NP=2 continuation is running from the immutable step-60000 checkpoint. Over steps 60000--62211, the restart seam is roundoff-continuous; mass flux, `fbcx`, and wall heat flux remain close, while tiny `wrms` trajectories later separate by up to `3.8914281162951948e-6`. This establishes local restart continuity, not long-horizon bitwise identity or external physical validation.
_Avoid_: Treating the running job as G4 completion, treating CPU/GPU or restart equivalence as OpenSBLI physical agreement, overwriting the source checkpoint, or ignoring the long-horizon `wrms` separation before G3 closure

**Post-P3 migration priority**:
