...
def step_budget(case, step, dt, rank, node, jacobian):
    def state(label, stage):
        return _active_array(path(case, label, stage, rank, step))[node][:2]

    def rhs(label, stage):
        snapshot = read_rhs_snapshot(path(case, label, stage, rank, step))
        im, jm, km, nq = snapshot.header
        return snapshot.values.reshape((im+1, jm+1, km+1, nq), order='F')[node][:2]/jacobian

    initial = state('pre_chemistry', 1)
    origin = state('pre_rhs', 1)
    end = state('post_chemistry', 2)
    transport = state('post_transport', 1)
    terms = {key: np.zeros(2) for key in ('convection_raw', 'convection_limiter',
    ...
    for stage, ((a, b, c), weight, update_weight) in enumerate(
        ...
        stages.append(dict(stage=stage, raw_convection_rho_momentum_rhs=raw.tolist(),
            limited_convection_rho_momentum_rhs=conv.tolist(),
            diffusion_rho_momentum_rhs=(full-conv).tolist(),
    ...
    return initial, end, terms, dict(step=step, stages=stages,
        closure_rho_momentum=closure.tolist())
...
def trajectory(root, backend, suffix, rank, node):
    ...
    for step in range(contract['start_step'], contract['start_step']+contract['updates']):
        start, end, local, stage_report = step_budget(
            case, step, contract['dt'], rank, node, jacobian)
        if initial is None:
            initial = start
            terms = {key: np.zeros(2) for key in local}
            terms['between_step_change'] = np.zeros(2)
        else:
