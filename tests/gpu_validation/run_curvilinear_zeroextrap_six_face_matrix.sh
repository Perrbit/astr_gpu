...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/curvilinear_zeroextrap_six_face_matrix}"
...
run_case() {
  ...
  ZERO_AXIS="$axis" OUT_DIR="$case_out" \
  ...
  BOUNDARY_ATOL="${BOUNDARY_ATOL:-1e-10}" \
    "$ROOT_DIR/tests/gpu_validation/run_curvilinear_zeroextrap_compare.sh"
