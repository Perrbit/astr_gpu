...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/mp2_derivative_benchmark_$STAMP}"
...
CANDIDATE_TIMINGS="$OUT_DIR/${CANDIDATE}_timings.tsv"
...
run_once() {
  ...
  local label timings run_dir log monitor stop monitor_pid start end status timing_count
  local timing_values retained_count wall max_memory max_util
  label="$mode"
...
run_once mixed_workspace "$CANDIDATE" warmup f
for repeat in $(seq 1 "$REPEATS"); do
  if (( repeat % 2 == 1 )); then
    run_once fp64 flux "$repeat" t
    run_once mixed_workspace "$CANDIDATE" "$repeat" t
  else
    run_once mixed_workspace "$CANDIDATE" "$repeat" t
    run_once fp64 flux "$repeat" t
  fi
...
mixed_bytes="$(awk -F= '/^ASTR_GPU_MIXED_WORKSPACE_BYTES=/ {print $2; exit}' "$OUT_DIR/${CANDIDATE}_run_1/run.log")"
fp64_bytes="$(awk -F= '/^ASTR_GPU_FP64_WORKSPACE_BYTES=/ {print $2; exit}' "$OUT_DIR/${CANDIDATE}_run_1/run.log")"
if [[ "$CANDIDATE" == "derivative" ]]; then
  workspace_elements="$(python3 -c "i,j,k=map(int,'$GRID'.split(',')); print((i+1)*(j+1)*(k+1)*12)")"
else
  workspace_elements="$(python3 -c "i,j,k=map(int,'$GRID'.split(',')); h=5; print((i+2*h+1)*(j+2*h+1)*(k+2*h+1)*9)")"
fi
