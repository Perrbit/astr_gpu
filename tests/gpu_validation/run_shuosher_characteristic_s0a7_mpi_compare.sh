...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/shuosher_characteristic_s0a7_mpi_compare}"
GRID="${GRID:-400,8,8}"
MAXSTEP="${MAXSTEP:-1}"
...
run_case() {
  ...
  (
    ...
    ASTR_SHUOSHER_SHOCK_X=0.d0 \
    ASTR_SHOCK_SENSOR_DUMP="$OUT_DIR/${target}_shock_sensor.dat" \
      mpirun -np 2 "$exe" run datin/input.shuosher > "${target}.log" 2>&1
  )
}
