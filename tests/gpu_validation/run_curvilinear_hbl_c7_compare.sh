...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/curvilinear_hbl_c7}"
IM="${IM:-64}"
JM="${JM:-64}"
KM="${KM:-8}"
NP="${NP:-1}"
...
MAXSTEP="$MAXSTEP" FEQCHKPT="$MAXSTEP" GRID_WARP_X="$WARP_X" GRID_WARP_Y="$WARP_Y" \
WALL_TEMPERATURE="$WALL_TEMPERATURE" CPU_GEOMETRY_DUMP="$CPU_GEOMETRY_DUMP" \
CONSCHM="$CONSCHM" UPPER_BCTYPE="${UPPER_BCTYPE:-51}" \
FIELD_ATOL="${FIELD_ATOL:-1e-10}" FIELD_RTOL="${FIELD_RTOL:-1e-10}" \
STATS_ATOL="${STATS_ATOL:-1e-10}" STATS_RTOL="${STATS_RTOL:-1e-10}" \
  "$ROOT_DIR/tests/gpu_validation/run_s1_hbl_s1c2_m5_similarity_field_compare.sh"
