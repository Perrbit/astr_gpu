#!/usr/bin/env python3
"""Compare CPU flowstate enstophy with GPU scalar-reduction output."""
...
def main() -> int:
    ...
    gpu_header, gpu = read_table(args.gpu, "gpu_enstophy.dat")
    if gpu_header != ["nstep", "time", "enstophy_gpu"]:
        raise ValueError(f"unexpected GPU enstophy header: {gpu_header}")

    nstep_col = cpu_header.index("nstep")
    time_col = cpu_header.index("time")
    enstophy_col = cpu_header.index("enstophy")
    ...
    lines = [
        ...
        "",
        "nstep time cpu_enstophy gpu_enstophy abs rel",
    ]
    for row in gpu:
        nstep = int(row[0])
        if nstep not in cpu_by_step:
            raise KeyError(f"GPU enstophy nstep {nstep} not present in CPU flowstate")
        cpu_row = cpu_by_step[nstep]
        cpu_value = float(cpu_row[enstophy_col])
