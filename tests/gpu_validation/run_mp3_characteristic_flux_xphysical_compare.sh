...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/mp3_characteristic_flux_xphysical_$STAMP}"
...
CANDIDATE_STATS_ATOL="${CANDIDATE_STATS_ATOL:-2e-6}"
CANDIDATE_STATS_RTOL="${CANDIDATE_STATS_RTOL:-0}"
CANDIDATE_FIELD_ATOL="${CANDIDATE_FIELD_ATOL:-2e-6}"
...
[[ ! -e "$OUT_DIR" ]] || { echo "refusing to overwrite MP3 x-physical evidence: $OUT_DIR" >&2; exit 2; }
...
prepare_case() {
  ...
  python3 "$ROOT_DIR/tests/gpu_validation/prepare_tgv_case.py" \
    ...
    --use-gpu "$use_gpu" --maxstep "$MAXSTEP" --feqchkpt "$FEQCHKPT" \
    --lfilter f --diffterm f --scheme 643e --conschm 543e --difschm 643e \
    --recon-schem 3 --lchardecomp t --grid "$GRID" --deltat "$DELTAT"
}
  ...
  (
    ...
    if [[ -n "$precision" ]]; then
      ...
      OMPI_MCA_btl=self,vader,tcp OMPI_MCA_osc=pt2pt \
        mpirun --oversubscribe -np "$NP" "$exe" run datin/input.shuosher > "$target.log" 2>&1
    else
      ASTR_FORCE_MPI_TOPOLOGY="$TOPOLOGY" \
      ASTR_SHOCK_SENSOR_DUMP="$OUT_DIR/${target}_shock_sensor.dat" \
      OMPI_MCA_coll='^hcoll,ucc' OMPI_MCA_pml=ob1 \
...
echo "MP3 characteristic_flux x-physical comparison passed: $OUT_DIR"
