...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/symmetry_phasec_mpirank_matrix}"
...
run_case() {
  ...
  case_out="$OUT_DIR/${axis}sym_np${np}_${tag}"

  BC_KIND=symmetry ZERO_AXIS="$axis" NP="$np" TOPOLOGY="$topology" \
