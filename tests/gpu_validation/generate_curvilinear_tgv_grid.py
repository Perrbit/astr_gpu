...
def mapped_grid(im: int, jm: int, km: int, amplitude: float) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    derivatives = np.empty(x.shape + (3, 3), dtype=np.float64)
...
def main() -> int:
    ...
    x, y, z, jacobian = mapped_grid(im, jm, km, args.amplitude)
    ...
    args.report.write_text(
        "\n".join(
            (
                "mapping: x=xi+a*sin(xi)*sin(eta), y=eta+a*sin(eta)*sin(zeta), z=zeta+a*sin(zeta)*sin(xi)",
                f"grid: {im},{jm},{km}",
