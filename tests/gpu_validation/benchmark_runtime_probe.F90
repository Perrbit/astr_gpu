program benchmark_runtime_probe
  ...
  implicit none
  character(32) :: gpu_arg,flow_arg,homogeneous_arg,boundary_arg,expected_arg,timing_arg
  logical :: use_gpu,homogeneous,periodic_boundary,expected,expected_timing
  integer :: ierr,rank
  ...
  call get_command_argument(6,timing_arg)
  ...
  expected_timing=trim(timing_arg)=='timing'
  ...
  if(benchmark_cpu_rk_timing_enabled().neqv.expected_timing) then
    write(*,'(A,I0)') 'benchmark CPU RK timing probe mismatch on rank ',rank
    call mpi_abort(MPI_COMM_WORLD,1,ierr)
  endif
