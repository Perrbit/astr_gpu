...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/air5_numq11_shock_tube}"
...
if ! [[ "$TOPOLOGY_X" =~ ^[1-9][0-9]*$ && "$TOPOLOGY_Y" =~ ^[1-9][0-9]*$ \
  && "$TOPOLOGY_Z" =~ ^[1-9][0-9]*$ ]] \
  || (( TOPOLOGY_X * TOPOLOGY_Y * TOPOLOGY_Z != NP )); then
  echo "numq11 shock-tube gate requires TOPOLOGY product equal to NP" >&2
  exit 2
fi
...
python3 "$ROOT_DIR/tests/gpu_validation/compare_q_validation_snapshots.py" \
  ...
  --report "$OUT_DIR/cpu_gpu_compare.txt" \
...
for mode in cpu gpu; do
  python3 "$ROOT_DIR/tests/gpu_validation/check_air5_numq11_shock_tube.py" \
    --prefix "$OUT_DIR/$mode/validation/air5" \
    --mechanism "$ROOT_DIR/chemMech/air5_kimjo12.json" \
    --report "$OUT_DIR/${mode}_shock_tube.txt"
done
