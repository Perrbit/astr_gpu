...
def main() -> None:
    ...
    worst = 0.0
    for cpu_prefix, gpu_prefix in (
        ("NSCBC_CPU_POLICY", "NSCBC_GPU_POLICY"),
        ("NSCBC_CPU_VISCOUS", "NSCBC_GPU_VISCOUS"),
    ):
        cpu = records(args.cpu, cpu_prefix)
        gpu = records(args.gpu, gpu_prefix)
        if cpu.keys() != gpu.keys():
            raise SystemExit(f"case mismatch: CPU={sorted(cpu)} GPU={sorted(gpu)}")

        for label in cpu:
            cpu_lambda, cpu_mask, cpu_values = cpu[label]
            gpu_lambda, gpu_mask, gpu_values = gpu[label]
            if not np.array_equal(cpu_mask, gpu_mask):
                raise SystemExit(
                    f"mask mismatch for {label}: {cpu_mask} != {gpu_mask}"
                )
            error = max(
                float(np.max(np.abs(cpu_lambda - gpu_lambda))),
                float(np.max(np.abs(cpu_values - gpu_values))),
            )
            worst = max(worst, error)
            if error > args.atol:
                raise SystemExit(
                    f"value mismatch for {label}: {error:.16e} > {args.atol:.16e}"
                )
