...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/curvilinear_wall42_np${NP}}"
...
case "$WALL_AXIS" in
  ...
  y)
    ...
    ;;
  z)
    printf 'WALL_AXIS=z is unsupported because CPU noslip_adibatic defines only ndir=1..4\n' >&2
    exit 2
    ;;
  *)
    printf 'WALL_AXIS must be x or y; got %s\n' "$WALL_AXIS" >&2
    exit 2
    ;;
esac
...
for mode in cpu gpu; do
  ...
  python3 "$ROOT_DIR/tests/gpu_validation/check_wall42_invariants.py" \
    --input "$invariant_input" \
    --report "$OUT_DIR/${mode}_wall42_invariants.txt" \
    --axis "$HDF_AXIS" \
    --gamma "${GAMMA:-1.4}" \
    --mach "${MACH:-0.1}" \
    --atol "${WALL_ATOL:-1e-10}"
done
