...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/profile_inflow11_mach_pressure}"
IM="${IM:-32}"
JM="${JM:-32}"
KM="${KM:-8}"
MAXSTEP="${MAXSTEP:-1}"
...
(
  cd "$OUT_DIR/cpu"
  ASTR_PROFILE_INFLOW_MODE=mach_pressure \
...
(
  cd "$OUT_DIR/gpu"
  ASTR_PROFILE_INFLOW_MODE=mach_pressure ASTR_FORCE_MPI_TOPOLOGY=1,1,1 \
...
for target in cpu gpu; do
  ...
  python3 "$ROOT_DIR/tests/gpu_validation/check_profile_inflow_mach_pressure_invariants.py" \
    --input "$input" --profile "$OUT_DIR/$target/datin/inlet.prof" \
    --mach "$MACH" --report "$OUT_DIR/${target}_inflow11_mach_pressure.txt"
done
