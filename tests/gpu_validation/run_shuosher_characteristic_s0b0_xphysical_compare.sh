...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/shuosher_characteristic_s0b0_xphysical}"
GRID="${GRID:-400,8,8}"
NP="${NP:-1}"
...
(
  ...
  ASTR_SHOCK_SENSOR_DUMP="$OUT_DIR_ABS/cpu_shock_sensor.dat" \
...
(
  cd "$OUT_DIR/gpu"
  ASTR_FORCE_MPI_TOPOLOGY="$TOPOLOGY" \
  ASTR_SHOCK_SENSOR_DUMP="$OUT_DIR_ABS/gpu_shock_sensor.dat" \
    mpirun -np "$NP" "$GPU_EXE" run datin/input.shuosher > gpu.log 2>&1
)

sensor_args=(
  --cpu "$OUT_DIR/cpu_shock_sensor.dat"
  --gpu "$OUT_DIR/gpu_shock_sensor.dat"
  --report "$OUT_DIR/shock_sensor_compare.txt"
  --atol "$SENSOR_ATOL"
  --rtol "$SENSOR_RTOL"
)
if (( NP > 1 )); then
  sensor_args+=(--rankwise)
fi
python3 "$ROOT_DIR/tests/gpu_validation/compare_shock_sensor.py" "${sensor_args[@]}"
