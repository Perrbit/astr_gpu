...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/curvilinear_hbl_c7_matrix}"
...
run_case() {
  ...
  printf 'pass label=%s np=%s topology=%s maxstep=%s grid=%sx%sx%s out=%s\n' \
    "$label" "$np" "$topology" "$maxstep" "${IM:-64}" "${JM:-64}" "$km" \
    "$OUT_DIR/$label" >> "$SUMMARY"
}

run_case np1_long 1 1,1,1 20 8
run_case np2_x 2 2,1,1 5 8
run_case np2_y 2 1,2,1 5 8
run_case np2_z 2 1,1,2 5 16
run_case np4_xy 4 2,2,1 1 8
