...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/shuosher_characteristic_s0a6_compare}"
GRID="${GRID:-400,8,8}"
MAXSTEP="${MAXSTEP:-1}"
...
(
  ...
  ASTR_SHOCK_SENSOR_DUMP="$OUT_DIR/cpu_shock_sensor.dat" \
    mpirun -np 1 "$CPU_EXE" run datin/input.shuosher > cpu.log 2>&1
)
...
(
  ...
  ASTR_SHOCK_SENSOR_DUMP="$OUT_DIR/gpu_shock_sensor.dat" \
    mpirun -np 1 "$GPU_EXE" run datin/input.shuosher > gpu.log 2>&1
)
