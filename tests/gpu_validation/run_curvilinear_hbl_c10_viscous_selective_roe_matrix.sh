...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/tests/gpu_validation/out/curvilinear_hbl_c10_viscous_selective_roe_matrix}"
...
run_case() {
  ...
  WALL_ATOL="${WALL_ATOL:-1e-12}" \
    "$ROOT_DIR/tests/gpu_validation/run_curvilinear_hbl_c10_viscous_selective_roe_compare.sh"
...
run_case np4_xz 4 2,1,2 3 16 f
run_case np4_yz 4 1,2,2 3 16 f
run_case np8_xyz 8 2,2,2 3 16 f

if [[ "${RUN_STRESS:-f}" == "t" ]]; then
  run_case np1_stress 1 1,1,1 100 8 f
  run_case np8_stress 8 2,2,2 100 16 f
fi
