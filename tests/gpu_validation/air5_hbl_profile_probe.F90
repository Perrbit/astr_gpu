program air5_hbl_profile_probe
  ...
  use chemistry_hbl_profile, only: air5_hbl_profile_type, &
    ...
    air5_hbl_profile_point_count,air5_hbl_profile_x_origin
  implicit none

  type(air5_hbl_profile_type) :: profile
  character(len=32) :: mode
  character(len=1024) :: path
  real(real64) :: y_min,y_max,y_sample,x_origin,q(air5_num_conservative)
  ...
  call air5_hbl_profile_x_origin(profile,x_origin,status)
  if(status/=air5_hbl_profile_status_ok) then
    write(*,'(I0)') status
    stop
  endif
  ...
  write(*,'(2(I0,1X),16(ES25.16E3,1X))') status, &
    air5_hbl_profile_point_count(profile),y_min,y_max,y_sample,rho,velocity, &
    pressure,temperature,tv,mass_fraction,sum(mass_fraction),x_origin
