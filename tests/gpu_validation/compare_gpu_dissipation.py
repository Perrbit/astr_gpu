#!/usr/bin/env python3
"""Compare CPU flowstate dissipation with GPU scalar-reduction output."""
...
def read_table(path: Path, filename: str) -> tuple[list[str], np.ndarray]:
    fpath = path / filename if path.is_dir() else path
    if not fpath.exists():
...
def main() -> int:
    ...
    cpu_header, cpu = read_table(args.cpu, "flowstate.dat")
    gpu_header, gpu = read_table(args.gpu, "gpu_dissipation.dat")
    if gpu_header != ["nstep", "time", "dissipation_gpu"]:
        raise ValueError(f"unexpected GPU dissipation header: {gpu_header}")

    nstep_col = cpu_header.index("nstep")
    time_col = cpu_header.index("time")
    diss_col = cpu_header.index("dissipation")
    ...
    lines = [
        ...
        "",
        "nstep time cpu_dissipation gpu_dissipation abs rel",
    ]
    for row in gpu:
        nstep = int(row[0])
        if nstep not in cpu_by_step:
            raise KeyError(f"GPU dissipation nstep {nstep} not present in CPU flowstate")
        cpu_row = cpu_by_step[nstep]
        cpu_value = float(cpu_row[diss_col])
